...
class ControlNode:
    ...
    def calibration(self):
        ...
        rospy.loginfo("Move the Robot 1 meter in 'x' axis")
        raw_input("Hit Enter when the robot is in place...")
        x = self.p[0]
        self.calFactor = 1/x
    ...
    def callback_pose(self, data):
        ...
        # self.p = np.arra([self.calFactor*data.pose.position.x,self.calFactor*data.pose.position.y,yaw])
        (_,_,yaw) = euler_from_quaternion(q)
